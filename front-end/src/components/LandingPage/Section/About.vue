...
// Ubah jadi computed agar teksnya reaktif saat ganti bahasa
const dropdownOptions = computed(() => [
  { value: "Bug Report", label: t("about.form.options.bug") },
  { value: "Feature Request", label: t("about.form.options.feature") },
  { value: "Report Content", label: t("about.form.options.content") },
  { value: "General Question", label: t("about.form.options.question") },
  { value: "Other", label: t("about.form.options.other") },
]);
...
const submitContact = async () => {
  if (!form.value.name || !form.value.email || !form.value.message) {
    Swal.fire({
      icon: "warning",
      title: t("about.form.alert.incomplete_title"),
      text: t("about.form.alert.incomplete_text"),
  ...
  try {
    ...
    if (response.ok) {
      Swal.fire({
        icon: "success",
        title: t("about.form.alert.success_title"),
        text: t("about.form.alert.success_text"),
        confirmButtonColor: "#9a203e",
      });
      // Reset form
      form.value = {
    ...
    Swal.fire({
      icon: "error",
      title: t("about.form.alert.error_title"),
      text: t("about.form.alert.error_text"),
      confirmButtonColor: "#9a203e",
    });
  } finally {
    ...
    <div
      ...
      <div
        ...
        <div
          ...
          <h3 class="text-xl font-bold text-white mb-3">{{ $t("about.features.amplify_title") }}</h3>
          <p class="text-sm text-gray-400 leading-relaxed group-hover:text-gray-300 transition-colors">
            {{ $t("about.features.amplify_desc") }}
          </p>
        </div>
        ...
        <div
          ...
          <h3 class="text-xl font-bold text-white mb-3">{{ $t("about.features.judgment_title") }}</h3>
          <p class="text-sm text-gray-400 leading-relaxed group-hover:text-gray-300 transition-colors">
            {{ $t("about.features.judgment_desc") }}
          </p>
        </div>
        ...
        <div
          ...
          <h3 class="text-xl font-bold text-white mb-3">{{ $t("about.features.trace_title") }}</h3>
          <p class="text-sm text-gray-400 leading-relaxed group-hover:text-gray-300 transition-colors">
            {{ $t("about.features.trace_desc") }}
          </p>
        </div>
      </div>
      ...
      <div class="mt-24 w-full max-w-3xl animate-fade-in-up relative z-10" style="animation-delay: 0.5s">
        <div
          ...
          <div class="text-center mb-10 relative z-10">
            <h2 class="text-3xl font-bold text-white mb-3 tracking-tight">{{ $t("about.form.title") }}</h2>
          ...
          <form @submit.prevent="submitContact" class="space-y-6 relative z-10">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div class="space-y-2 group/input">
                <label class="text-xs font-bold text-[#9a203e] uppercase tracking-widest ml-1">
                  {{ $t("about.form.name_label") }}
                </label>
                <input
                  v-model="form.name"
                  type="text"
                  :placeholder="$t('about.form.name_placeholder')"
              ...
              <div class="space-y-2 group/input">
                <label class="text-xs font-bold text-[#9a203e] uppercase tracking-widest ml-1">
                  {{ $t("about.form.email_label") }}
                </label>
                <input
                  v-model="form.email"
                  type="email"
                  :placeholder="$t('about.form.email_placeholder')"
                  class="w-full bg-[#0f0505] border border-gray-700/50 rounded-xl px-4 py-4 text-white placeholder:text-gray-600 focus:border-[#9a203e] focus:outline-none focus:ring-1 focus:ring-[#9a203e] transition-all" />
              </div>
            </div>
            ...
            <div class="space-y-2 group/input">
              <label class="text-xs font-bold text-[#9a203e] uppercase tracking-widest ml-1">
                {{ $t("about.form.message_label") }}
              </label>
              <textarea
                v-model="form.message"
                rows="5"
                :placeholder="$t('about.form.message_placeholder')"
            ...
            <button
              ...
              <span v-if="isLoading" class="flex items-center gap-2">
                ...
                {{ $t("about.form.sending") }}
              </span>
              <span v-else class="flex items-center gap-2">
                {{ $t("about.form.submit_btn") }}
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  class="h-4 w-4"
                  fill="none"
      ...
      <div class="mt-12 w-full max-w-2xl text-center animate-fade-in-up relative z-10" style="animation-delay: 0.6s">
        <div class="flex flex-col items-center gap-4">
          <h3 class="text-white font-bold text-lg">{{ $t("about.support.title") }}</h3>
      ...
      <div class="mt-16 text-center relative z-10 animate-fade-in-up" style="animation-delay: 0.7s">
        <p class="text-[10px] text-gray-500 uppercase tracking-[0.3em] mb-4">{{ $t("about.support.credit") }}</p>
